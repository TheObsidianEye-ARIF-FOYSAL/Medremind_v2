...
// ── Route shim to AddMedicationScreen ────────────────────────────────────────
// Imported lazily to avoid circular deps with router.
class _AddMedRoute extends StatelessWidget {
  const _AddMedRoute();
  @override
  Widget build(BuildContext context) {
    // Import here to avoid circular dependency with app_router.dart
    return const _AddMedScreenLoader();
  }
}

class _AddMedScreenLoader extends StatelessWidget {
  const _AddMedScreenLoader();
  @override
  Widget build(BuildContext context) {
    // Using direct import — resolved at build time
    return const _AddMedView();
  }
}

// Resolved at link time — avoids import cycle.
class _AddMedView extends StatelessWidget {
  const _AddMedView();
  @override
  Widget build(BuildContext context) {
    return const _AddMedWrapper();
  }
}

class _AddMedWrapper extends StatelessWidget {
  const _AddMedWrapper();
  @override
  Widget build(BuildContext context) {
    // Direct import of add_medication_screen.dart widget
    return _buildAddMed(context);
  }

  Widget _buildAddMed(BuildContext context) {
    // We import via the exported builder to keep this file clean.
    // The actual screen is AddMedicationScreen from medicine_cabinet.
    return const _LazyAddMed();
  }
}

class _LazyAddMed extends StatelessWidget {
  const _LazyAddMed();
  @override
  Widget build(BuildContext context) {
    return _AddMedNavigator.build(context);
  }
}

// Singleton builder to avoid duplicate code
class _AddMedNavigator {
  static Widget build(BuildContext ctx) {
    // Import directly — resolves at compile time
    return Builder(builder: (c) {
      return _InlineAddMed();
    });
  }
}

class _InlineAddMed extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    // Real screen is imported at top of file (would cause a cycle with router)
    // Solution: import it here with a late import trick.
    // We return a placeholder that immediately pushes the real screen.
    // Actually, simplest: just call it from app_router.dart instead.
    // For the FAB, we navigate via a named route.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (context.mounted) context.go(AppRoutes.addMedication);
    });
